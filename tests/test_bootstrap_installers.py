...
PY
        ...
        MDS_SITL_GCS_API_TOKEN_FILE="/run/secrets/sitl_gcs_api_token"
        GCS_ENV="production"
        ...
        grep -q 'set-environment -t MDS-GCS MDS_SITL_GCS_API_TOKEN_FILE /run/secrets/sitl_gcs_api_token' "$tmpdir/tmux.log"
        ...
        grep -q 'export MDS_SITL_GCS_API_TOKEN_FILE=/run/secrets/sitl_gcs_api_token;' <<<"$tmux_prefix"
...
def test_gcs_server_launcher_exports_sitl_runtime_env_from_system_config():
    ...
    assert "new runtime/security settings" in launcher_text
...
def test_post_sync_runtime_restart_runs_directly_for_root_without_sudo():
    result = run_bash(
        f"""
        tmpdir="$(mktemp -d)"
        repo_dir="$tmpdir/repo"
        bin_dir="$tmpdir/bin"
        home_dir="$tmpdir/home/root"
        mkdir -p "$repo_dir" "$bin_dir" "$home_dir/logs"
...
def test_services_lib_core_service_order_excludes_embedded_wifi_manager():
    ...
    assert '/usr/bin/systemctl daemon-reload' in services_text
    assert '/usr/bin/systemctl reenable git_sync_mds.service' in services_text
