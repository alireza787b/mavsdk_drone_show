...
### Fixed
- Drone Show runtime validation now rebuilds and retries its safe GET polling after transient transport resets, and its SITL acceptance script now recreates the selected SITL fleet between internal mode runs, so long multi-mode validation no longer flakes out on a stale keep-alive or on the previous run's landed geometry
