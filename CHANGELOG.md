...
- Swarm Trajectory waypoint authoring now labels derived timing/speed values explicitly (`Derived waypoint arrival time`, `Derived required speed`) so the modal and waypoint panel show which values are operator-owned versus planner-derived instead of relying on disabled fields or post-hoc warnings
