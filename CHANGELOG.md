...
### Fixed
- Drone Show runtime validation now rebuilds and retries its safe GET polling after transient transport resets, recreates the selected SITL fleet between internal mode runs, and re-checks launch readiness before one bounded retry when an immediate post-reset show submit briefly comes back `HTTP 400`, so long multi-mode validation no longer flakes out on stale transport, bad landed geometry, or short re-stage timing races
