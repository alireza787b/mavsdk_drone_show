...
- Swarm Trajectory review/dispatch wording now treats partial outputs as attention-state review packages instead of generic launch-ready results, and the handoff back to command control is labeled `Open Dashboard Preflight` to reinforce that launch approval still belongs to the dashboard preflight gate
...
- Swarm Trajectory authoring terminology is now precise about leg-vs-waypoint ownership: the UI and docs use `Route entry time`, `Waypoint arrival time`, `Preferred leg speed`, `Required leg speed`, and `Entry heading` / `Arrival heading` consistently, so operators no longer have to infer whether a field belongs to the route anchor, the inbound leg, or the arrival posture
