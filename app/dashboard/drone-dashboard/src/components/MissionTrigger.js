            ...
            div className = "mission-trigger-container" > {
                    ...
                    !selectedMission && ( <
                        div className = "mission-cards" > {
                            Object.entries(missionTypes).map(([key, value]) => ( <
                                MissionCard key = { value }
                                missionType = { value }
                                icon = {
                                    value === DRONE_MISSION_TYPES.DRONE_SHOW_FROM_CSV ? '🛸' : value === DRONE_MISSION_TYPES.CUSTOM_CSV_DRONE_SHOW ? '🎯' : value === DRONE_MISSION_TYPES.SMART_SWARM ? '🐝🐝🐝' : '🚫'
                                }
                                label = { key === 'NONE' ? 'Cancel Mission' : key.replace(/_/g, ' ') }
                                onClick = {
                                    () => handleMissionSelect(value) }
...

                {
                    selectedMission && selectedMission !== DRONE_MISSION_TYPES.NONE && ( <
                        MissionDetails missionType = { selectedMission }
                        icon = {
                            selectedMission === DRONE_MISSION_TYPES.DRONE_SHOW_FROM_CSV ? '🛸' : selectedMission === DRONE_MISSION_TYPES.CUSTOM_CSV_DRONE_SHOW ? '🎯' : '🐝🐝🐝'
                        }
