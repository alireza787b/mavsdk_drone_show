...
const BriefingExport = ({ configData, originLat, originLon, setOriginLat, setOriginLon }) => {
    ...
    return ( <
        div className = "briefing-export-container" > { /* Buttons for Exporting to KML and Printing Mission Briefing */ } <
        div className = "briefing-actions" >
        <
        button className = "export-kml"
        onClick = { exportToKML }
        title = "Export drone positions to KML" >
        <
        FontAwesomeIcon icon = { faDownload }
        /> Export to Google Earth (KML) <
        /button> <
        button className = "print-mission"
        onClick = { handlePrint }
        title = "Print the mission briefing" >
        <
        ...
        { /* Origin Modal */ } {
            showOriginModal && ( <
                OriginModal isOpen = { showOriginModal }
                onClose = {
                    () => setShowOriginModal(false) }
