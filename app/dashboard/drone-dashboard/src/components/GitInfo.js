  ...
  return (
    <div className={`git-info-simple ${expanded ? 'expanded' : ''}`} onClick={() => setExpanded(!expanded)}>
      <div className="git-summary">
        <div className="git-repo-line">{gitInfo.repo}</div>
        <div className="git-branch-line">
          ...
          <span className="commit-hash">{gitInfo.commit || '...'}</span>
        </div>
      </div>
      ...
      {expanded && (
        <div className="git-details">
          <div className="detail-line">
            ...
            <span className="detail-val">{gitInfo.raw?.author_name || 'N/A'}</span>
          </div>
          <div className="detail-line">
            <span className="detail-key">Date:</span>
            <span className="detail-val">{gitInfo.raw?.commit_date ? new Date(gitInfo.raw.commit_date).toLocaleDateString() : 'N/A'}</span>
          </div>
          <div className="detail-line message-line">
            <span className="detail-key">Message:</span>
            <span className="detail-val">{gitInfo.raw?.commit_message}</span>
          </div>
          {gitInfo.raw?.uncommitted_changes && gitInfo.raw.uncommitted_changes.length > 0 && (
            <div className="changes-line">
              <span className="changes-count">{gitInfo.raw.uncommitted_changes.length}</span> uncommitted
            </div>
          )}
        </div>
      )}
