...
Implementation for Slices 1-5 is complete in local working trees and has passed
focused local tests and reviewer re-checks. It is not yet released, privately
merged, deployed, SITL-validated end to end, or hardware-validated.

Slice 6 remains the active release/deploy gate:

1. Commit/tag/push official MDS.
2. Run heavy frontend build/browser checks on
   Hetzner.
3. Merge/sync the private client repo after public releases are clean.
4. Deploy private GCS.
5. Run SITL validator before switching back to real mode.
6. Ask boards to be connected only after GCS is healthy, then dry-run/apply board
...
| 6. Release/deploy/board sync | Pending. Local tests/reviews are done; release, private deploy, SITL, and hardware validation are still outstanding. |
...
The public sidecar releases are complete. Official MDS, private client MDS,
private deploy, SITL validation, and hardware validation are still pending.

| Repo | State |
| --- | --- |
| Official MDS | Working tree has implementation and blocker fixes; uncommitted, untagged, unpushed. |
...
| Private client MDS | Not merged in this resume segment. Public releases must happen first. |

## Remaining Debt And Notes

- Final public leak scan is still required before push/release. `gitleaks` and
...
- API docs still mention raw GPS altitude as an untrusted MSL fallback in legacy
  payload examples; this is not a blocker because the new source-aware altitude
  report is the operator-facing policy surface.
