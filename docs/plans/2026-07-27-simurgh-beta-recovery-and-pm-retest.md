...
| Official tip | `5afafe7b` (runtime release `69f40fae`) |
...
| 2026-07-27 | Readiness-routing correction and final handoff | Official runtime `69f40fae` is tagged `v5.5.113-simurgh-readiness-routing`, with closing docs on official `main` at `5afafe7b`; the private runtime replay is `638c03a8f`, with closing docs on private `main` at `af82878b`. Official serial gates passed 363 broader tests and 152 Simurgh route tests, dashboard evals passed 33/33, and provider smoke passed. Private focused routes passed 7/7 and dashboard evals passed 33/33. Production APIs/dashboard are healthy after restart, the existing SITL container was preserved, and authentication remains enabled. | PM may run the final authenticated private-client test; readiness questions should remain read-only, while explicit actions use guarded confirmation |
