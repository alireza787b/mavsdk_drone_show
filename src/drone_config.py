...
class DroneConfig:
    ...
    def calculate_setpoints(self):
        ...
        if self.target_drone:
            ...
            print(f"Setpoint updated | Position: [N:{self.position_setpoint_NED.get('north'):.1f}, E:{self.position_setpoint_NED.get('east'):.1f}, D:{self.position_setpoint_NED.get('down'):.1f}] | Velocity: [N:{self.velocity_setpoint_NED.get('vel_n'):.1f}, E:{self.velocity_setpoint_NED.get('vel_e'):.1f}, D:{self.velocity_setpoint_NED.get('vel_d'):.1f}] | following drone {self.target_drone.hw_id}, with offsets [N:{self.swarm.get('offset_n', 0):.1f},E:{self.swarm.get('offset_e', 0):.1f},Alt:{self.swarm.get('offset_alt', 0):.1f}]")
