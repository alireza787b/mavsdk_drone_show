...
MAVLINK_ANYWHERE_REF="${MAVLINK_ANYWHERE_REF:-${MDS_MAVLINK_ANYWHERE_REF:-${MDS_DEFAULT_MAVLINK_ANYWHERE_REF:-v3.0.10}}}"
...
display_mavlink_menu() {
    ...
    echo -e "${CYAN}├────────────────────────────────────────────────────────────────────────────┤${NC}"
    echo -e "${CYAN}│${NC}                                                                            ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}    ${GREEN}[1]${NC} Auto-configure (Recommended defaults)                               ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}        - Installs mavlink-router if needed                                 ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}        - Auto-detects a UART device and uses standard MDS endpoints        ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}        - Adds the GCS push endpoint only when your config provides one     ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}        - Best for normal FC-over-UART companion setups                     ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}                                                                            ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}    ${YELLOW}[2]${NC} Guided configuration                                                ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}        - Step-by-step mavlink-anywhere prompts                             ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}        - Choose UART, USB serial, or UDP input and set devices manually    ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}                                                                            ${CYAN}│${NC}"
...
display_mavlink_instructions() {
    ...
    echo -e "${CYAN}│${NC}     ${DIM}- UART Device: choose the real FC-facing device for your board${NC}           ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}     ${DIM}  Examples: /dev/ttyS0, /dev/ttyAMA0, /dev/ttyTHS1, /dev/ttyUSB0${NC}       ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}     ${DIM}- Baud Rate: match the FC MAVLink serial port${NC}                          ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}     ${DIM}  Holybro Pixhawk CM4 baseboard: TELEM2 usually 921600${NC}                ${CYAN}│${NC}"
