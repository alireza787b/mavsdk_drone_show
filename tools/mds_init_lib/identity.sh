...
get_existing_configured_hwid() {
    local configured_id=""

    configured_id=$(get_local_env_value "MDS_HW_ID" "")
    if validate_drone_id "$configured_id" >/dev/null 2>&1; then
        printf '%s\n' "$configured_id"
        return 0
    fi

    if [[ -f "${MDS_NODE_IDENTITY_FILE}" ]] && command -v jq &>/dev/null; then
        configured_id=$(jq -r '.hw_id // ""' "${MDS_NODE_IDENTITY_FILE}" 2>/dev/null || echo "")
        if validate_drone_id "$configured_id" >/dev/null 2>&1; then
            printf '%s\n' "$configured_id"
            return 0
        fi
...
setup_local_env() {
    ...
    local mavlink_repo_url="${MAVLINK_ANYWHERE_REPO_URL:-${MDS_MAVLINK_ANYWHERE_REPO_URL:-}}"
    local mavlink_ref="${MAVLINK_ANYWHERE_REF:-${MDS_MAVLINK_ANYWHERE_REF:-}}"
    local mavlink_repo_url_explicit="${MAVLINK_ANYWHERE_REPO_URL_EXPLICIT:-false}"
    local mavlink_ref_explicit="${MAVLINK_ANYWHERE_REF_EXPLICIT:-false}"
...

    {
        ...
        printf '# This file is loaded as the host-local runtime override layer\n'
        printf '# Do not commit this file to git - it contains drone-specific configuration\n\n'
        printf '# Hardware ID (required)\n'
        printf 'MDS_HW_ID=%s\n' "$drone_id"
        printf '\n# Canonical runtime mode for this host\n'
        printf 'MDS_MODE=real\n'
        printf '\n# Optional connectivity backend for this host\n'
        printf 'MDS_CONNECTIVITY_BACKEND=%s\n' "${MDS_CONNECTIVITY_BACKEND:-${MDS_DEFAULT_CONNECTIVITY_BACKEND:-none}}"
        ...
        if [[ "${mavlink_management_mode}" == "fleet-merge" || "${mavlink_management_mode}" == "fleet-strict" ]]; then
            ...
            if [[ "${mavlink_repo_url_explicit}" == "true" && -n "${mavlink_repo_url}" ]]; then
                printf 'MDS_MAVLINK_ANYWHERE_REPO_URL=%s\n' "${mavlink_repo_url}"
            fi
            if [[ "${mavlink_ref_explicit}" == "true" && -n "${mavlink_ref}" ]]; then
                printf 'MDS_MAVLINK_ANYWHERE_REF=%s\n' "${mavlink_ref}"
            fi
        fi
...
remove_local_env_value() {
    ...
    grep -v "^${key}=" "${MDS_LOCAL_ENV}" > "${tmp_file}" || true
    mv "${tmp_file}" "${MDS_LOCAL_ENV}"
}
...
run_identity_phase() {
    ...
    # Check for existing canonical hardware ID
    local existing_id
    existing_id=$(get_existing_configured_hwid)
