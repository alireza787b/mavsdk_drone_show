...
class DroneTelemetry(BaseModel):
    ...
    global_position_valid: bool = Field(False, description="Whether PX4 has published a valid GLOBAL_POSITION_INT coordinate")
    global_position_timestamp_ms: int = Field(0, description="Last valid global position sample timestamp (Unix ms)")
    global_position_age_ms: Optional[int] = Field(None, description="Age of the last valid global position sample")
...
class DroneGitSyncRuntimeStatus(BaseModel):
    ...
    service_reload_message: str = Field("", description="Systemd unit reload summary from the last node sync")
    deferred_unit_actions: List[str] = Field(default_factory=list, description="Unit apply actions deferred until next invocation, restart, or boot")
